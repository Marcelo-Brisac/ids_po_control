---
name: multi-search
description: "Fallback web search using multiple providers (Serper, Tavily, Exa, You.com). Use ONLY when the firecrawl-search skill is unavailable, errors out, or doesn't fit the request type. Prefer firecrawl-search for all web search tasks first."
---

# Multi-Search (Fallback)

This is a **fallback** search skill. Always prefer the **`firecrawl-search`** skill for web search tasks first. Use this skill only when firecrawl-search is unavailable, returns errors (e.g. HTTP 402 out-of-credit), or doesn't support the specific search type you need (e.g. Exa neural similarity, You.com livecrawl).
