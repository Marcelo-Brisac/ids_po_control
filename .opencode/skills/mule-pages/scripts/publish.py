...
def create_page(env: dict, page_name: str, category: str, with_database: bool = False) -> str:
    """Create a new page. Returns domain."""
    resp = page_api(env, "POST", "/api/page", {
        "name": page_name,
        "agent": AGENT_NAME,
        "category": category,
        "session_id": env.get("MULE_SESSION_ID", ""),
...
def ensure_database_ready(env: dict, domain: str, timeout: int = 180) -> None:
    """Attach database if needed, then poll until ready.

    Handles three cases:
    - No database attached: POST to attach, wait for provisioning (~30-60s)
    - Database attached but provisioning: wait for ready
    - Database already ready: return immediately
    """
    resp = page_api(env, "GET", f"/api/page/{domain}/database")
    state = resp.get("data", {}).get("state", "")

    if not state:
        info("Attaching database...")
        resp = page_api(env, "POST", f"/api/page/{domain}/database")
        if resp.get("code") != "ok":
            fatal(f"Error attaching database: {json.dumps(resp)}")
        state = "provisioning"

    if state == "ready":
        return
    if state == "error":
        fatal("Database provisioning failed")
    ...
    while time.time() < deadline:
        ...
        if state == "error":
            fatal("Database provisioning failed")
        time.sleep(5)
    fatal(f"Database provisioning timed out after {timeout}s")


def list_versions(env: dict, domain: str) -> list[str]:
    """Return list of version strings, e.g. ['V1', 'V2']."""
    resp = page_api(env, "GET", f"/api/page/{domain}/artifact?page_number=1&page_size=100")
    if resp.get("code") != "ok":
        return []
...
def main():
    ...
    parser.add_argument("--with-database", action="store_true",
                        help="Provision a TiDB database for this dynamic page")
    ...
    if args.with_database and args.category != "dynamic":
        fatal("--with-database requires --category dynamic")
    ...
    if domain:
        ...
        domain = create_page(env, page_name, args.category, args.with_database)
        version = "V1"
        info(f"Created page: {domain}")

    # Step 2: Ensure database is provisioned (if requested)
    if args.with_database:
        ensure_database_ready(env, domain)
