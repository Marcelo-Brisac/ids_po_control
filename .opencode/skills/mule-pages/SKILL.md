...
| `--with-database` | `false` | Attach a database to a dynamic page. See the `mule-page-database` skill for the full workflow. |
...
### Dynamic page with a database

See the `mule-page-database` skill. The publish step still goes through this skill (pass `--with-database` to `publish.py`), but the workflow constraints, code patterns, and error handling live there.
