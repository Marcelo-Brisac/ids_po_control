...
name: html-report
description: >
  Generate premium single-file HTML data reports and analytical
  publications with rich data visualization. Trigger when the
  request is fundamentally about data analysis, research reporting,
  or metrics presentation — content where charts, tables, and visual
  storytelling add clear value (research reports / 研报, KPI or
  metrics overviews, market or competitive analysis with data, white
  papers with statistics, dashboard-style summaries, performance
  reviews with numbers). Do NOT trigger for creative writing,
  general Q&A, coding help, document format conversions, or any task
  where the primary deliverable is prose rather than data-driven
  analysis. Key distinction: if the core value of the output is in
  its DATA and VISUAL PRESENTATION, trigger this skill; if it is in
  PROSE and NARRATIVE, do not. Stack: ECharts (SVG), TailwindCSS,
  GSAP ScrollTrigger, Google Fonts. Output: single .html file.
license: MIT
---
