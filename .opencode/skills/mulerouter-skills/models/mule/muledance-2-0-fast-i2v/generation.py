"""Muledance 2.0 Fast Image-to-Video generation endpoint.

Lower latency / lower cost variant; capped at 720p.

Path: /vendors/mulerouter/v1/muledance-2.0-fast/image-to-video/generation.
"""
...
ENDPOINT = ModelEndpoint(
    model_id="mule/muledance-2.0-fast-i2v",
    action="generation",
    provider="mule",
    model_name="muledance-2.0-fast-i2v",
    description=(
        "Fast variant of Muledance 2.0 Image-to-Video (lower latency / lower cost). "
        "First-frame + optional last-frame interpolation, 480p or 720p, 4-15s."
    ),
    input_types=[InputType.IMAGE, InputType.TEXT],
    output_type=OutputType.VIDEO,
    api_path="/vendors/mulerouter/v1/muledance-2.0-fast/image-to-video/generation",
    result_key="videos",
    ...
    parameters=[
        ...
        ModelParameter(
            ...
            type="string",
            description=(
                "Optional last-frame image (URL or Base64). Model interpolates "
                "first-to-last. First-frame aspect ratio wins."
            ),
            required=False,
        ),
        ModelParameter(
            ...
            type="string",
            description=(
                "Optional text prompt describing motion/scene. "
...
class Muledance20FastI2VGeneration(BaseModelEndpoint):
    """Muledance 2.0 Fast Image-to-Video generation endpoint."""
...
def main() -> int:
    """CLI entry point."""
    return Muledance20FastI2VGeneration().run()
