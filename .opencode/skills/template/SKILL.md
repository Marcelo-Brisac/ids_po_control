...
Read `<project-root>/template/AGENTS.md` template file to know current template content(it may be empty).
...
You shall update `<project-root>/template/AGENTS.md` template file.
...
Call the `save_template` MCP tool with the template's semantic fields. The MCP packages `/workspace/template` together with the user-created skills under `/workspace/.opencode/skills/`, uploads the archive, and registers the template — you do not need to package or list skills yourself.
